...
- **Payment configuration:** no Stripe Payment Link was supplied. At the owner’s request, both Stripe bindings are temporarily absent from Specific so they cannot block tonight’s deployment. The integration remains in code and fails closed with a disabled button until the real Payment Link and webhook secret are added tomorrow. No fake checkout URL is used.
