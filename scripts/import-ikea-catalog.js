...
class IKEACatalogImporter {
  ...
  async createOrUpdateProduct(productData) {
    ...
    const existingQuery = `SELECT id, updated_at FROM products WHERE ikea_item_number = $1`;
  ...
  async createProductAliases(productId, productData) {
    // Create normalized item number alias
  ...
  async createOrGetManualDocument(manualUrl) {
    ...
    return { id: docResult.rows[0].id, asset_id: assetId };
  }
  ...
  async importProducts(researchData) {
    ...
    for (const productData of researchData.products) {
      try {
